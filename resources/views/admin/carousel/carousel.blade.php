...
@section('content')
    <div class="mws-panel grid_8">
        <div class="mws-panel-header">
            <span><i class="icon-list-ul"></i> 轮播图管理</span>
        </div>
                ...
                <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info">
                    ...
                    <tr role="row">
                        <th   class="sorting_asc" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" style="width: 10px;" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" >
                            编号</th>
                        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" style="width: 80px;" aria-label="Browser: activate to sort column ascending">图片</th>

                        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" style="width: 25px;" aria-label="Engine version: activate to sort column ascending">标题</th>
                        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" style="width: 25px;" aria-label="Engine version: activate to sort column ascending">网址</th>
                        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" style="width: 25px;" aria-label="Engine version: activate to sort column ascending">是否显示</th>
                    ...
                    @foreach($result as $arr)
                        <tr class="active">
                            <th scope="row"  style="line-height:80px;">{{$loop->iteration}}</th>
                            <td style="line-height:80px;"><img src="{{asset('admin/image/carousel/'.$arr->pic)}}" height="70" width="70"></td>
                            <td>{{$arr->title}}</td>
                            <td style="width:300px">{{$arr->url}}</td>
                            <td style="line-height:80px;">@if($arr->onsale=='0')"no"@else"yes"@endif</td>
                            <td style="line-height:80px;"><a href="{{asset('admin/carousel/edit/'.$arr->id)}}">修改</a>&nbsp;
                                <a href="{{asset('admin/carousel/del/'.$arr->id)}}">删除</a></td>
                        </tr>
                    @endforeach
                    </tbody>
